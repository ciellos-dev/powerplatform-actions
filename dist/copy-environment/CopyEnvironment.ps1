...
function Copy-Request {
    ...
    process {
        $copyRequest = [pscustomobject]$Parameters
    }
    end {
...
function Copy-Environment {
    ...
    process {
        $copyRequest = Copy-Request -SourceEnvironmentId $SourceEnvironmentId `
        ...
        $response = Copy-PowerAppEnvironment -EnvironmentName $TargetEnvironmentInfo.EnvironmentId `
                -CopyToRequestDefinition $copyRequest `
                -WaitUntilFinished $true `
                -TimeoutInMinutes 60 -Verbose

         if ($null -eq $response -or $response.Code -ne '200') {
            Write-Error $response
            Write-Error "The copy environment task failed..."
            return
        }
        ...
        if ($DisableAdminMode) {
            ...
            if (!$isAdminModeDisabled) {
                Write-Error "The copy environment task is completed. but, the system could not disable the admin mode..."
                return
            }
...
try {
    ...
    $copyType = Get-VstsInputWithDefault -Name "CopyType" -taskJsonFile $taskJson
    ...
    Write-Host 'Target Environment details'
    Write-AuthLog -AuthInfo $authInfo
    $targetOrganizationInfo = Get-OrgInfo -Credential $authInfo.Credential -TenantId $authInfo.TenantId `
                             -EnvironmentUrl $targetEnvironmentUrl -AuthType $authInfo.AuthType -InformationAction Continue

    if ([String]::IsNullOrWhiteSpace($friendlyName)) {
        $friendlyName = $targetOrganizationInfo.FriendlyName
    }

    $SourceOrganizationInfo = Get-OrgInfo @authInfo
    Write-Host 'Source Environment details'
    Write-AuthLog -AuthInfo $authInfo -InformationAction Continue
    Copy-Environment $authInfo `
                    -SourceEnvironmentId $SourceOrganizationInfo.EnvironmentId `
                    -TargetEnvironmentUrl $targetEnvironmentUrl `
                    -TargetEnvironmentInfo $targetOrganizationInfo `
                    -TargetEnvironmentName $friendlyName `
                    -CopyType $copyType `
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDXuWqXsgnFW++i
# ZdmBSHKXTbsNdyuaqLhFnrXYzgiyRqCCDYEwggX/MIID56ADAgECAhMzAAACUosz
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgd7SpiXJZ
# wujP8KruEMCxjNFbKn//EbYdPaDqzwckRW8wNAYKKwYBBAGCNwIBDDEmMCSgEoAQ
# AFQAZQBzAHQAUwBpAGcAbqEOgAxodHRwOi8vdGVzdCAwDQYJKoZIhvcNAQEBBQAE
# ggEApAO5rwXfnIjxqS/xrxM8V3+g8pB0rCglVsN7DO3sqRm9mPifq7iSOYuRXOMM
# kPX6Vgyh29J/6SFVb5iR3BKt+BCs6gPxQHzc4kI2PZoUb0UiDewu5CJBGzZAekiH
